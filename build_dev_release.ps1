...
if (-not $SkipRelease) {
    Invoke-Step "Create or update DEV release" {
        powershell -ExecutionPolicy Bypass -File .\scripts\release_manager.ps1 `
            -Repo $Repo `
            -Tag $tag `
            -Channel dev `
            -NotesPath $notesPath `
            -ExePath $exePath `
            -VersionPath $distReleaseManifestPath `
            -ShaPath $shaPath `
            -AssetName $assetName
    }
}
